...
let prsiGame = {
    ...
    // cardsMap: { 'eso': [5, 16, 20, 31], '7': [4, 14, 18, 26], '8': [3, 15, 19, 32], '9': [2, 17, 21, 33], '10': [8, 10, 23, 30], 'spodek': [6, 12, 24, 28], 'svršek': [7, 13, 25, 27], 'král': [9, 11, 22, 29] },
    cardsMap: {
        '2': 9,
        '3': 8,
        '4': 7,
...
function showCard(card, activePlayer) {
    if (activePlayer['score'] === 0) {
        ...
        cardImage.src = `photos/prsi-photos/${cardsMap}.png`;
...
function win() {
    if (player) { //if player has no cards == YOU WIN!
...
function matchingConditions() {
    if (cardSuit[žaludy] === 'žaludy' || //if zaludy == zaludy playerUp plays again
